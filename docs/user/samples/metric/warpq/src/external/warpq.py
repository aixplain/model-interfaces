...
class WARPQ(object):
    ...
    def __init__(self, mapping_model, sr=16000, n_mfcc=13, fmax=5000, patch_size=0.4, sigma=[[1, 0], [0, 3], [1, 3]], apply_vad=True):
        """
        ...
        if not sr in (8000, 16000):
            raise ValueError("Sampling rate of audio files should be either 8 kHz or 16 kHz.")
        ...
        # Load mapping model
        self.load_model(self.mapping_model)
    ...
    def load_model(self, zip_path):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            if isH5Exist:
                ...
                print(
                    "Model is based on deep neural networks with a sequential stack regressor from Keras. It is trained using "
                    + os.path.basename(zip_path)[0:-4]
                    + " database \n"
                )
    ...
    def load_audio(self, ref_path, test_path):
        ...
        if ref_path[-4:] == ".wav":
            speech_Ref, sr_Ref = librosa.load(ref_path, sr=self.sr)
        else:
            if ref_path[-4:] == ".SRC":  # For ITUT database if applicable
                speech_Ref, sr_Ref = sf.read(ref_path, format="RAW", channels=1, samplerate=16000, subtype="PCM_16", endian="LITTLE")
                if sr_Ref != self.sr:
                    speech_Ref = librosa.resample(speech_Ref, sr_Ref, self.sr)

        # 2) Load coded audio
        if test_path[-4:] == ".wav":
            speech_Coded, sr_Coded = librosa.load(test_path, sr=self.sr)
        else:
            if test_path[-4:] == ".OUT":  # For ITUT database if applicable
                speech_Coded, sr_Coded = sf.read(test_path, format="RAW", channels=1, samplerate=16000, subtype="PCM_16", endian="LITTLE")
                if sr_Coded != self.sr:
                    speech_Coded = librosa.resample(speech_Coded, sr_Coded, self.sr)
    ...
    def evaluate(self, ref_path, test_path):
        ...
        if self.apply_vad:
            # VAD for Ref speech
            vact1 = vad(speech_Ref, self.sr, fs_vad=self.sr_vad, hop_length=self.hop_size_vad, vad_mode=self.aggresive)

            speech_Ref = speech_Ref[vact1 == 1]

            # VAD for Coded speech
            vact2 = vad(speech_Coded, self.sr, fs_vad=self.sr_vad, hop_length=self.hop_size_vad, vad_mode=self.aggresive)
        ...
        cols = int(self.patch_size / (self.hop_length / self.sr))
