...
import tempfile
import os
from docs.user.samples.metric.bleu.src.model import BLEU

INPUTS_PATH="docs/user/samples/metric/bleu/tests/inputs.json"
OUTPUTS_PATH="docs/user/samples/metric/bleu/tests/outputs.json"
...
def test_bleu(inputs, outputs, testcase):
    scorer = BLEU()
    metric = 'bleu'
...
def test_bleu_aggregation(inputs, outputs):
    scorer = BLEU()
    metric = 'bleu'
    results = []
