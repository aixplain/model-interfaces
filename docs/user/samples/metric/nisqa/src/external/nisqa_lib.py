...
class NISQA_DE(nn.Module):
    ...
    def __init__(
        ...
        self.fuse = Fusion(
            in_feat=self.time_dependency.fan_out, fuse_dim=de_fuse_dim, fuse=de_fuse,
        )
...
class DFF(nn.Module):
    ...
    def __init__(
        self, cnn_seg_length, ms_n_mels, dropout, fc_out_h=4096,
    ):
        super().__init__()
        self.name = "DFF"
...
class LSTM(nn.Module):
    ...
    def forward(self, x, n_wins):
        ...
        x = pack_padded_sequence(x, n_wins, batch_first=True, enforce_sorted=False)
...
class Pooling(nn.Module):
    ...
    def __init__(
        self, d_input, output_size=1, pool="att", att_h=None, att_dropout=0,
    ):
        super().__init__()
...
class Alignment(torch.nn.Module):
    ...
    def __init__(
        self, att_method, apply_att_method, q_dim=None, y_dim=None,
    ):
        super().__init__()
...
def predict_mos(model, ds, bs, dev, num_workers=0):
    ...
    with torch.no_grad():
        y_hat_list = [
            [model(xb.to(dev), n_wins).cpu().numpy(), yb.cpu().numpy()]
...
def calc_mapping(
    dfile_db, mapping=None, dcon_db=None, target_mos=None, target_ci=None, pred=None,
):
    """
...
def get_librosa_melspec(
    file_path, sr=48e3, n_fft=1024, hop_length=80, win_length=170, n_mels=32, fmax=16e3
):
    """
