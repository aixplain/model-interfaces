...
from aixplain.model_schemas.schemas.metric_input import ReferencelessTextGenerationMetricInput, MetricAggregate
...
MODEL_NOT_FOUND_ERROR = """
    ...
    aws s3 cp --recursive https://benchmarksdata.s3.amazonaws.com/models/wmt-large-qe-estimator-1719/_ckpt_epoch_1.ckpt .
"""

class CometQEMetric(ReferencelessTextGenerationMetric):
    params = {
        "casing" : ["Cased", "Uncased"],
        "punctuation" : ["Punctuated", "Not Punctuated"]
    }

    def __init__(self, name="comet-qe", *args, **kwargs):
        self.all_possible_configs = [dict(zip(self.params, vals)) for vals in itertools.product(*self.params.values())]
        super().__init__(name, *args, **kwargs)

    def load(self, model_name:str="wmt21-comet-qe-da", batch_size:int=16, gpus:int=0):
    ...
    def run_metric(self, request: Dict[str, List[ReferencelessTextGenerationMetricInput]]) -> Dict[str, List[ReferencelessTextGenerationMetricOutput]]:
        """Scoring Function for Unbabel Comet-QE Metric
        ...
        for inp in inputs:
            ...
            for current_config in self.all_possible_configs:
                details_dict = {}
                hypotheses, sources = inp.hypotheses.copy(), inp.sources.copy()
                ...
                data = [{ "src": sources[i], "mt": hypotheses[i] } for i in range(len(hypotheses))]
                seg_scores, corpus_score = self.model.predict(data, batch_size=self.batch_size, gpus=self.gpus)
...
if __name__ == "__main__":
    metric = CometQEMetric(AssetResolver.asset_uri())
