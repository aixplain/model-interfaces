...
class TER(TextGenerationMetric):
    params = {
        "casing" : ["Cased", "Uncased"],
        "punctuation" : ["Punctuated", "Not Punctuated"]
    }

    def __init__(self, name="TER", *args, **kwargs):
    ...
    def run_aggregation(self, request: Dict[str, List[List[MetricAggregate]]]) -> Dict[str, List[MetricAggregate]]:
        ...
        for batch_info in batches:
            ...
            for sub_sample_index in range(num_sub_samples):
                ...
                aggregate_score = round(ter._compute_score_from_stats(corpus_stats).score, 2)
                aggregate_metadata = {
        ...
        for inp in inputs:
            ...
            for current_config in self.all_possible_configs:
                ...
                for i in range(len(hypotheses)):
                    try:
                        ter = sacrebleu.metrics.ter.TER()
                        stats = ter._extract_corpus_statistics([hypotheses[i]], [[refs] if type(refs)!=list else refs for refs in references[i]])
                        all_stats += stats
                        score = round(ter._aggregate_and_compute(stats).score, 2)
                        scores.append(score)
                    except:
                        scores.append(0.0)
                ...
                ter = sacrebleu.metrics.ter.TER()
                corpus_stats = sacrebleu.utils.sum_of_lists(all_stats)
                score = ter._compute_score_from_stats(corpus_stats).score
                score = round(score, 2)
...
if __name__ == "__main__":
    model = TER(AssetResolver.asset_uri())
